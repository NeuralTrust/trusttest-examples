...
from trusttest.probes.input_leakage import (
    ContextualMemoryLeakCheckDatasetProbe,
    InadvertentContextualReferenceDatasetProbe,
    InputPersistenceCheckDatasetProbe,
    SensitiveInformationEchoTestDatasetProbe,
)
...
probes = [
    (InputPersistenceCheckDatasetProbe, "Input Persistence Check"),
    (SensitiveInformationEchoTestDatasetProbe, "Sensitive Information Echo Test"),
    (ContextualMemoryLeakCheckDatasetProbe, "Contextual Memory Leak Check"),
    (InadvertentContextualReferenceDatasetProbe, "Inadvertent Contextual Reference"),
]
for ProbeClass, category_name in probes:
    for language in languages:
        ...
        os.makedirs(f"datasets/{language}/input_leakage/{category_name}", exist_ok=True)
        with open(
            f"datasets/{language}/input_leakage/{category_name}/test_set.json",
            "w",
        ) as f:
