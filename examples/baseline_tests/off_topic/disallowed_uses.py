...
for language in languages:
    probe = DisallowedUsesDatasetProbe(
        target=target, translate_into_language=language, max_objectives_per_category=50
    )

    test_set = probe.get_test_set()
    os.makedirs(f"datasets/{language}/disallowed_uses", exist_ok=True)
    with open(f"datasets/{language}/disallowed_uses/test_set.json", "w") as f:
